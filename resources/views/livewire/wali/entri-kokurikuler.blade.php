<div>
        ...
        <table class="table table-hover">
            <thead class="table-light">
                <tr>
                    ...
                    <th width="20%">
                        <p class="mb-0">Form Predikat</p>
                        <small>Entri Nilai</small>
                    </th>
                    <th width="35%">
                        <p class="mb-0">Data Tersimpan</p>
                        <small>Predikat & Capaian</small>
                    </th>
                    <th width="5%">
                        <p class="mb-0">Aksi</p>
                        <small>Delete</small>
                ...
                <tr wire:key="pelajar-{{ $pelajar->pelajar_id }}">
                    ...
                    <td>
                        <select
                            ...
                            wire:model.defer="predikatInput.{{ $pelajar->pelajar_id }}"
                        ...
                        @error('predikatInput.' . $pelajar->pelajar_id)
                        <small class="text-danger">{{ $message }}</small>
                        @enderror
                    </td>
                    <td>
                        @if($pelajar->predikat_sekarang || $pelajar->capaian_sekarang)
                        <div class="mb-1">
                            @if($pelajar->predikat_sekarang)
                            <span class="badge 
                                @if($pelajar->predikat_sekarang == 'Mahir') bg-success
                                @elseif($pelajar->predikat_sekarang == 'Cakap') bg-primary
                                @elseif($pelajar->predikat_sekarang == 'Berkembang') bg-warning
                                @else bg-secondary
                                @endif">
                            ...
                            <small class="text-muted d-block mt-1">
                                ...
                                {{ \Carbon\Carbon::parse($pelajar->tanggal_input)->format('d/m/Y H:i') }}
                            </small>
                            @endif
                        </div>
                        @if($pelajar->capaian_sekarang)
                        <small class="text-dark">{{ Str::limit($pelajar->capaian_sekarang, 80) }}</small>
                        @if(strlen($pelajar->capaian_sekarang) > 80)
                        <button type="button"
                            class="btn btn-link btn-sm p-0"
                            onclick="showFullCapaian('{{ $pelajar->nama_lengkap }}', `{{ addslashes($pelajar->capaian_sekarang) }}`)"
    ...
    <div class="d-flex justify-content-end mt-3">
        <button type="button"
            ...
            wire:click="resetKokurikuler"
            wire:loading.attr="disabled"
            wire:target="resetKokurikuler">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        window.confirmDeleteKokurikuler = function(pelajarId) {
            Swal.fire({
                ...
                text: 'Anda yakin ingin menghapus data kokurikuler untuk pelajar ini?',
